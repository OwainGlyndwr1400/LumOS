...
import { patchSettings, getOverdrive, setOverdrive, type OverdriveStatus } from "../api";
import type {
  STTProvider,
  SettingsUpdate,
  Telemetry,
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-1 truncate font-mono text-2xs text-muted">
                  {overdrive?.enabled
                    ? `cloud big-brain · ${overdrive.nvidia_model}`
                    : overdrive?.available
                      ? "tap to flip Lumos onto NVIDIA's cloud big-brain"
                      : "set LUMOS_NVIDIA_API_KEY in .env + restart to enable"}
                </div>
              </div>
              <button
                type="button"
