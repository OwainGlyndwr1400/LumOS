...
import {
  ...
  PELE_TOKEN_STORAGE_KEY,
  getOverdrive,
  patchSettings,
  setOverdrive,
  type OverdriveStatus,
} from "../api";
import type {
  STTProvider,
  SettingsUpdate,
...
export default function SettingsModal({
  ...
  const [overdrive, setOd] = useState<OverdriveStatus | null>(null);
  const [odBusy, setOdBusy] = useState(false);
  const [apiToken, setApiToken] = useState(() => {
    if (typeof window === "undefined") return "";
    return window.localStorage.getItem(API_TOKEN_STORAGE_KEY) ?? "";
  });
  const [peleToken, setPeleToken] = useState(() => {
    if (typeof window === "undefined") return "";
    return window.sessionStorage.getItem(PELE_TOKEN_STORAGE_KEY) ?? "";
  });

  useEffect(() => {
    getOverdrive().then(setOd).catch(() => {});
  }, []);

  const toggleOverdrive = async () => {
    if (!overdrive?.available || odBusy) return;
    setOdBusy(true);
    try {
      setOd(await setOverdrive(!overdrive.enabled));
    } catch (e) {
      setError(String(e));
    } finally {
      setOdBusy(false);
  ...
  const switchProvider = async (p: string) => {
    // Can't switch the cloud provider mid-flight — toggle Overdrive off first.
    if (overdrive?.enabled || odBusy) return;
    setOdBusy(true);
    try {
      await patchSettings({ overdrive_provider: p });
      setOd(await getOverdrive());
    } catch (e) {
      setError(String(e));
    } finally {
      setOdBusy(false);
  ...
  const updatePeleToken = (value: string) => {
    setPeleToken(value);
    if (typeof window === "undefined") return;
    const trimmed = value.trim();
    if (trimmed) window.sessionStorage.setItem(PELE_TOKEN_STORAGE_KEY, trimmed);
    else window.sessionStorage.removeItem(PELE_TOKEN_STORAGE_KEY);
  };
    ...
    >
      ...
      >
        ...
        <div className="space-y-5">
          {/* ── NVIDIA OVERDRIVE — one-tap cloud big-brain ── */}
          <div
            className={
              "rounded-md border p-3 transition-colors " +
              (overdrive?.enabled ? "border-accent" : "border-line")
            }
          >
              ...
              <div className="min-w-0">
                <div
                  className={
                    ...
                    (overdrive?.enabled ? "text-accent" : "text-muted")
                  }
                >
                  ⚡ overdrive
                </div>
                <div className="mt-1 truncate font-mono text-2xs text-muted">
                  {overdrive?.enabled
                    ? `cloud big-brain · ${overdrive.cloud_model ?? overdrive.nvidia_model}`
                    : overdrive?.available
                      ? `tap to flip Lumos onto the ${overdrive?.provider ?? "nvidia"} cloud big-brain`
                      : `set the ${(overdrive?.provider ?? "nvidia").toUpperCase()} API key in .env + restart`}
                </div>
              </div>
              <button
                type="button"
                role="switch"
                aria-checked={!!overdrive?.enabled}
                disabled={odBusy || !overdrive?.available}
                onClick={toggleOverdrive}
                className={
                  "relative h-5 w-10 shrink-0 rounded-full border transition-colors " +
                  (overdrive?.enabled ? "border-accent bg-accent/30" : "border-line bg-bg") +
                  (!overdrive?.available ? " cursor-not-allowed opacity-40" : "")
                }
              >
                <span
                  className={
                    "absolute top-0.5 size-3.5 rounded-full transition-all " +
                    (overdrive?.enabled ? "left-[22px] bg-accent" : "left-0.5 bg-muted")
                  }
                />
              </button>
            </div>
              ...
              {(["nvidia", "gemini"] as const).map((p) => {
                ...
                return (
                  <button
                    key={p}
                    type="button"
                    disabled={overdrive?.enabled || odBusy}
                    onClick={() => switchProvider(p)}
                    className={
                      "flex-1 rounded border px-2 py-1 font-mono text-2xs uppercase tracking-widest transition-colors " +
                      (active
                        ? "border-accent text-accent"
                        : "border-line text-muted hover:text-fg") +
                      (overdrive?.enabled ? " cursor-not-allowed opacity-40" : "")
                    }
                  >
                    {p}
                  </button>
          ...
          <div className="rounded-md border border-line p-3">
            ...
            <input
              ...
              autoComplete="off"
              placeholder="empty for localhost dev"
              className="w-full rounded-sm border border-line bg-bg px-2 py-1.5 font-mono text-2xs text-fg outline-none focus:border-accent/50"
            />
          </div>
          ...
          <div className="rounded-md border border-line p-3">
            ...
            <input
              type="password"
              value={peleToken}
              onChange={(e) => updatePeleToken(e.target.value)}
              autoComplete="off"
              placeholder="session only; required for protected actions"
              className="w-full rounded-sm border border-line bg-bg px-2 py-1.5 font-mono text-2xs text-fg outline-none focus:border-accent/50"
            />
            <div className="mt-1.5 font-mono text-2xs text-muted">
              kept in this browser tab only; the server value stays in .env
            </div>
          </div>
