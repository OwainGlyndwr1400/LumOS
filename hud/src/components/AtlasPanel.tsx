...
import type { AtlasData, DoneEvent, SigillumGeometry } from "../types";
...
export default function AtlasPanel({ lastDone, collapsed, onToggle, width }: Props) {
  ...
  const fgRef = useRef<ForceGraphMethods<NodeObject<AtlasNodeData>, LinkObject<AtlasNodeData, { source: string; target: string; weight: number }>> | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      nodeMeshes.current.forEach((mesh, id) => {
        ...
        // Sleek constellation look (operator call 2026-06-11): keep nodes near
        // their spawn scale (~1) so the link filaments stay visible — the old
        // cbrt(log2)*3.2 inflated every node 5-7× a few frames after load and
        // buried the web. Gentle log term keeps big clusters reading bigger.
        const baseSize = 1.0 + Math.log2(n.size + 1) * 0.12;
  ...
  const getNodeLabel = (n: any): string => {
    const sig = n.sigillum as SigillumGeometry | undefined;
    const locus = sig
      ? `${sig.ring} - sector ${sig.sector} - ${sig.theta_deg.toFixed(1)}deg`
      : "free cluster";
    return `<div style="background:rgba(19,19,22,0.96);border:1px solid #1f1f24;padding:8px 10px;font-family:'IBM Plex Mono',monospace;font-size:11px;color:#ebebef;max-width:300px;border-radius:3px">
      ...
      <div style="color:#8a8a92;margin-top:5px;font-size:9px">${escapeHtml(locus)}</div>
    </div>`;
  };
      ...
      <div ref={containerRef} className="relative min-h-0 flex-1">
        {error ? (
          ...
          <ForceGraph3D
            ...
            linkColor={() => "rgba(122,184,176,0.34)"}
            linkWidth={0.4}
            linkOpacity={0.6}
