...
interface Props {
  ...
  onVoiceMacro?: (text: string) => boolean | Promise<boolean>;
}
...
export default function Composer({
  ...
  const handleTranscript = useCallback(
    (transcript: string) => {
      const maybeHandled = onVoiceMacro?.(transcript);
      if (maybeHandled instanceof Promise) {
        void maybeHandled.then((handled) => {
          if (!handled) appendTranscript(transcript);
        });
        return;
      }
      if (!maybeHandled) appendTranscript(transcript);
    },
    [appendTranscript, onVoiceMacro],
  );
