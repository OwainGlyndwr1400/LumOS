...
export interface SigillumGeometry {
...
export interface LoagaethCoordinate {
  leaf: number;
  row: number;
  column: number;
  domain: string;
...
export interface AtlasData {
  ...
  sigillum?: { inner: number; outer: number };
}
...
export interface UREVMState {
  ...
  phase?: "torque" | "observer_shell";
  shell_tick?: number;
  torque_fraction?: number;
  toggle_torque?: number;
  null_ledger?: {
    R: number;
    I: number;
    R_bits: string;
  ...
  ternary_register?: {
    ...
    t_observer: number;
    trit_str: string;
    ternary_value: number;
    phasor_imbalance: number;
    theta_deg: number;
    dominant_arm: "none" | "real" | "time" | "observer";
    balanced: boolean;
    shell_trit: number | null;
  };
  kuramoto_order?: {
    resolution_fill: number;
    identity_n: number;
    knowledge_n: number;
    n_states: number;
    resolution_limit: number;
    r: number;
    mean_theta_rad: number;
    hit_count: number;
    scale: number;
  } | null;
