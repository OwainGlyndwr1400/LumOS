...
const DEFAULT_VOICE_MACROS: MacroCatalogItem[] = [
  {
    id: "zacar_od_zamran",
...
function normalizeVoiceMacro(text: string): string {
...
function formatMacroResult(result: MacroExecuteResponse): string {
  const payload = result.payload;
  if (result.id === "zacar_od_zamran") {
    const health = asRecord(payload.health);
    const dream = asRecord(payload.dream);
    const urevm = asRecord(payload.urevm);
    const soul = asRecord(payload.soul);
    const atlas = asRecord(payload.atlas);
    return [
      "Voice macro: ZACAR OD ZAMRAN",
      "Diagnostic overlay compiled.",
      "",
      `node: ${String(health.node ?? "Lumos")} - auth ${String(health.auth ?? "unknown")} - PELE ${health.pele_configured ? "configured" : "locked"}`,
      `dream pending: ${String(dream.pending ?? "unknown")}`,
      `ure-vm: tick ${String(urevm.tick ?? "?")} - cycle ${String(urevm.cycle_position ?? "?")} - phase ${String(urevm.phase ?? "none")}`,
      `soul: ${String(soul.harmonic_band ?? "unknown")}`,
      `atlas: ${String(atlas.clusters ?? 0)} clusters`,
    ].join("\n");
  }
  if (result.id === "micma_adoian_mad") {
    const soul = asRecord(payload.soul);
    return [
      "Voice macro: MICMA ADOIAN MAD",
      "Bounded harmonic reset complete.",
      "",
      "runtime cache: cleared",
      "ure-vm: reset",
      "memory files: untouched",
      "FAISS indexes: untouched",
      `baseline: ${String(payload.baseline_hz ?? 1260)} Hz`,
      `soul: ${String(soul.harmonic_band ?? "1260 Hz baseline")}`,
    ].join("\n");
  }
  return `Voice macro executed: ${result.id}`;
}
...
export default function App() {
  ...
  const [voiceMacros, setVoiceMacros] =
    useState<MacroCatalogItem[]>(DEFAULT_VOICE_MACROS);
  ...
  useEffect(() => {
    fetchMacroCatalog()
      .then((items) => {
        if (items.length > 0) setVoiceMacros(items);
      })
      .catch(() => setVoiceMacros(DEFAULT_VOICE_MACROS));
  }, []);
  ...
  const handleVoiceMacro = useCallback(
    async (text: string): Promise<boolean> => {
      const normalized = normalizeVoiceMacro(text);
      const macro = voiceMacros.find(
        (m) => normalizeVoiceMacro(m.phrase) === normalized,
      );
      ...
      const assistantMsg: ChatMessage = {
        id: assistantId,
        role: "assistant",
        content: `executing voice macro: ${macro.label}`,
        timestamp: Date.now(),
      };
      setMessages((m) => [...m, userMsg, assistantMsg]);

      try {
        const result = await executeMacro({ id: macro.id });
        const content = formatMacroResult(result);
        setMessages((m) =>
          m.map((msg) => (msg.id === assistantId ? { ...msg, content } : msg)),
        );
        void fetchTelemetry().then(setTelemetry).catch(() => {});
        void refreshDreamStatus();
      } catch (e) {
        const content =
          `Voice macro failed: ${macro.label}\n\n` +
          (String(e).includes("403")
            ? "PELE ring required. Open Tune and enter the PELE token for this browser tab."
            : String(e));
        setMessages((m) =>
          m.map((msg) =>
            msg.id === assistantId ? { ...msg, content, error: String(e) } : msg,
          ),
        );
      }
      return true;
    },
    [refreshDreamStatus, voiceMacros],
  );
  ...
  const handleDawnBriefing = useCallback(async () => {
    ...
    try {
      const r = await apiFetch("/api/events/briefing", { method: "POST" });
      if (r.status === 403) {
        window.alert(
  ...
  const handleRunDream = useCallback(async () => {
    ...
    try {
      ...
      if (String(e).includes("403")) {
        window.alert(
          "PELE ring required.\n\nOpen Tune and enter the PELE token for this browser tab.",
        );
        return;
      }
