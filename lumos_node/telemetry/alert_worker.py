...
from ..enochian import enrich_event
...
def _write_state(s: Settings, st: dict[str, Any]) -> None:
    ...
    try:
        # Atomic: temp + rename so a crash mid-write can't corrupt the state
        # file. A truncated read would reset the alert-dedup map (spurious wake)
        # and the daily-cap counter.
        tmp.write_bytes(orjson.dumps(st, option=orjson.OPT_INDENT_2))
        os.replace(tmp, path)
    except OSError as e:
        log.warning("alert.state_write_failed", error=str(e))


# In-process append counter per log path — gates the full read+prune so we don't
# readlines() the whole file on every append. Seeded once per process from the
# real line count so pruning stays correct across restarts.
...
def _append_log(s: Settings, entry: dict[str, Any]) -> None:
    ...
    try:
        ...
        return
    # Cheap gate: use an in-process counter instead of readlines() on every
    # append. Seed from the real line count on the first append this process.
    key = str(p)
    n = _append_counts.get(key)
    if n is None:
        try:
            with p.open("rb") as f:
                n = sum(1 for _ in f)
        ...
        n += 1
    _append_counts[key] = n
    if n <= _LOG_CAP * 2:
        return
    # Over cap — rewrite the last _LOG_CAP lines via temp+rename so a crash
    # mid-prune can't corrupt the log.
    try:
        with p.open("rb") as f:
            lines = f.readlines()
        ...
        os.replace(tmp, p)
        _append_counts[key] = len(lines[-_LOG_CAP:])
    except OSError:
        pass
...
async def _evaluate_alerts(settings: Settings) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for ev in _evaluate_cosmic(snap, settings):
            try:
                ...
                log.debug("alert.record_skipped", source="cosmic", error=str(e))
                continue
        ...
        if mil.get("ok"):
            for ac in mil.get("aircraft", []):
                try:
                    ...
                    log.debug("alert.record_skipped", source="military_air", error=str(e))
                    continue
    except Exception as e:  # noqa: BLE001
            ...
            if civ.get("ok"):
                ...
                for ac in civ.get("aircraft", []):
                    try:
                        ...
                        log.debug("alert.record_skipped", source="aircraft", error=str(e))
                        continue
        except Exception as e:  # noqa: BLE001
        ...
        if gps.get("ok"):
            for z in gps.get("zones", []):
                try:
                    ...
                    log.debug("alert.record_skipped", source="gps_jamming", error=str(e))
                    continue
    except Exception as e:  # noqa: BLE001
        ...
        if sats.get("ok"):
            for st in sats.get("satellites", []):
                try:
                    ...
                    log.debug("alert.record_skipped", source="recon_satellite", error=str(e))
                    continue
    except Exception as e:  # noqa: BLE001
    ...
    try:
        for v in maritime.snapshot_vessels(lat, lon, settings.alert_ship_radius_km):
            try:
                ...
                log.debug("alert.record_skipped", source="vessel", error=str(e))
                continue
    except Exception as e:  # noqa: BLE001
            ...
            if board.get("ok"):
                ...
                for t in board.get("calls", []):
                    try:
                        ...
                        log.debug("alert.record_skipped", source="train", error=str(e))
                        continue
        except Exception as e:  # noqa: BLE001
    ...
    # ── GDACS global disasters (Phase 44) — edge-triggered inside the module
    # (fires once per NEW event at/above level, once more on escalation).
    if settings.alert_gdacs_enabled:
        try:
            from . import gdacs
            trips.extend(await gdacs.evaluate_gdacs(settings))
        except Exception as e:  # noqa: BLE001
            log.info("alert.gdacs_failed", error=str(e))

    # ── FIRMS satellite fire hotspots within alert_fire_radius_km. Per-cell ids
    # dedup via the monitor cooldown; a source cooldown throttles the kind.
    if settings.alert_fire_enabled:
        try:
            from . import fires
            trips.extend(await fires.evaluate_fires(settings))
        except Exception as e:  # noqa: BLE001
            log.info("alert.fire_failed", error=str(e))

    # ── ALL satellites overhead (Phase 44) — every bird above the cut, not just
    # recon (recon_satellite already covers those). OFF by default: Starlink is a
    # firehose. Source cooldown throttles the kind.
    if settings.alert_sat_all_enabled:
        try:
            trips.extend(await satellites.evaluate_all_sats_overhead(settings))
        except Exception as e:  # noqa: BLE001
            log.info("alert.sat_all_failed", error=str(e))

    # ── IODA internet outages — country-level blackouts (edge-triggered).
    if settings.alert_outage_enabled:
        try:
            from . import outages
            trips.extend(await outages.evaluate_outages(settings))
        except Exception as e:  # noqa: BLE001
            log.info("alert.outage_failed", error=str(e))
    ...
    return [enrich_event(t) for t in trips]
...
async def _poll_once(settings: Settings, bus: EventBus) -> None:
        ...
        "trips": [
            {
                "id": t["id"],
                "kind": t["kind"],
                "sentinel": t.get("sentinel"),
                "system_wake_code": t.get("system_wake_code"),
            }
            for t in trips
        ],
    ...
    # GeoSentinel Phase 44 source cooldowns (fire + all-satellites). Both can
    # produce a steady stream of distinct identities (new hotspots as fires
    # spread; every Starlink as the sky rolls), so throttle each kind as a whole
    # — same pattern as rail/aircraft/severe-wx. GDACS + outages are edge-
    # triggered in-module, so they need no source cooldown here. 0 = off.
    for _kind, _cd_min, _stamp in (
        ("wildfire", settings.alert_fire_cooldown_minutes, "last_wildfire_wake_unix"),
        ("satellite_overhead", settings.alert_sat_all_cooldown_minutes, "last_sat_all_wake_unix"),
    ):
        _cd = _cd_min * 60
        if _cd > 0 and any(t["kind"] == _kind for t in fresh):
            _last = float(state.get(_stamp) or 0.0)
            if now - _last < _cd:
                _held = [t["id"] for t in fresh if t["kind"] == _kind]
                fresh = [t for t in fresh if t["kind"] != _kind]
                audit[f"{_kind}_held"] = _held
                log.info(
                    f"alert.{_kind}_cooldown",
                    held=_held, mins_left=round((_cd - (now - _last)) / 60, 1),
                )
    ...
    if any(t["kind"] == "wildfire" for t in fresh):
        state["last_wildfire_wake_unix"] = now
    if any(t["kind"] == "satellite_overhead" for t in fresh):
        state["last_sat_all_wake_unix"] = now
    _write_state(settings, state)
    _append_log(settings, audit)
    log.info("alert.woke", kinds=kinds, n=len(fresh))

    # Phase 45.3 — self-updating entity memory. PURE-CODE queueing (NOT the LLM
    # turn): drop any unknown satellites/vessels/disasters from this wake onto
    # the enrichment queue; a separate worker looks them up later. The wake has
    # already spoken — autonomy still ends at speaking. Fully suppressed so an
    # enrichment hiccup can never affect the wake path.
    if settings.enrichment_enabled:
        with contextlib.suppress(Exception):
            from ..enrichment import queue_from_trips
            queue_from_trips(fresh, settings)

    # Phase 45.5 — Standing Orders. The wake has spoken; now run the operator's
    # SIGNED, pre-authorized recipes for any matching trip (deterministic code,
    # not the model deciding). Fully suppressed so a recipe can never break the
    # wake path. Gated + signature-checked inside fire_orders.
    if settings.standing_orders_enabled:
        with contextlib.suppress(Exception):
            from ..standing_orders import fire_orders
            await fire_orders(fresh, settings)
