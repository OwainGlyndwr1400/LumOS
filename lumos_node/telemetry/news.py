...
Mirrors the feed set used by a sibling SIGINT-news layer:
...
# Hard cap on a single feed/preview body before we materialize + parse it.
# Legit RSS/Atom feeds and t.me previews sit well under this; anything larger is
# rejected so a hostile or runaway body can't blow up memory or the XML parser.
...
def _parse_feed(xml_text: str, feed_url: str) -> list[dict]:
    ...
    """
    # XXE / billion-laughs guard, mirroring telemetry/gdacs.py: both attack
    # classes require a DTD (<!DOCTYPE/<!ENTITY). Legitimate RSS/Atom never
    # carries one, so reject any document declaring a DTD before parsing.
    head = xml_text[:4096]
    if "<!DOCTYPE" in head or "<!ENTITY" in head:
        log.warning("telemetry.news.dtd_rejected", url=feed_url)
        return []
    try:
...
async def _get_text(client: httpx.AsyncClient, url: str, **kwargs) -> str | None:
    ...
    try:
        ...
        if len(r.content) > _MAX_FEED_BYTES:
            log.info("telemetry.news.body_too_large", url=url, size_bytes=len(r.content))
            return None
        return r.text
