...
For a 50 km radius around Erydir, the box is well under 1 sq° → 1 credit.
"""
...
async def _get_access_token(client: httpx.AsyncClient) -> str | None:
    ...
    if not access:
        # Log only the OAuth error fields, never the whole third-party body —
        # the raw token-endpoint payload can carry a token or other sensitive
        # fields we must not write to logs.
        err = payload.get("error") if isinstance(payload, dict) else None
        err_desc = payload.get("error_description") if isinstance(payload, dict) else None
        log.warning("opensky.auth_no_token", error=err, error_description=err_desc)
        return None
