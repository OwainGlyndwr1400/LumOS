...
def _hhmm(v: Any) -> str | None:
    """HH:MM from a datetime or ISO-ish string; None if unusable."""
    if v is None:
        return None
    if hasattr(v, "strftime"):
        return v.strftime("%H:%M")
    s = str(v)
    return s[11:16] if "T" in s and len(s) >= 16 else None
...
def _fmt_grid(d: dict[str, Any]) -> str | None:
    ...
    if ph.get("ruler"):
        ...
        if phase and phase != "unknown":
            bits += f" {phase}"
        # Planetary-hour window (how long this hour runs) — HUD shows it; pings didn't.
        start, end = _hhmm(ph.get("hour_start_utc")), _hhmm(ph.get("hour_end_utc"))
        if start and end:
            bits += f" ({start}–{end}Z)"
        parts.append(bits)
    ...
    if moon.get("illumination_percent") is not None:
        ...
        sign = moon.get("zodiac_sign") or ""
        seg = f"moon {round(moon['illumination_percent'])}%{(' ' + nm) if nm else ''}"
        if sign:
            seg += f" in {sign}"   # zodiac sign — HUD shows it, pings didn't
        parts.append(seg)
    ...
    if reg.get("above_horizon") is not None:
        ...
        # Label BOTH numbers — a bare "-24.5°" once got narrated as "azimuth"
        # in a wake. Altitude is signed; azimuth is 0-360° from north.
        bits: list[str] = []
        if isinstance(alt, (int, float)):
            bits.append(f"alt {alt:+.1f}°")
        if isinstance(az, (int, float)):
            bits.append(f"az {az:.1f}°")
            if reg["above_horizon"] and 85.0 <= az <= 95.0:
                bits.append("in eastern gate")  # Sphinx window, az 85-95°E
        parts.append(f"Regulus {state}" + (f" ({', '.join(bits)})" if bits else ""))
    ...
    # Sidereal time + which planets are above the horizon — HUD shows both,
    # pings didn't. The astro block the operator reads on Discord while away.
    sid = d.get("sidereal_time")
    if sid:
        parts.append(f"sidereal {str(sid).split('.')[0]}")
    vis = d.get("visible_planets") or []
    names = ", ".join(p.get("name", "") for p in vis if p.get("name"))
    if names:
        parts.append(f"planets up: {names}")
    return " · ".join(parts) if parts else None
