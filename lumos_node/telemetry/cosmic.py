...
Endpoints (all real, checked Jul 2026):
  NOAA SWPC:
    ...
    - Solar wind Bz/Bt (summary):  /products/summary/solar-wind-mag-field.json
...
# Query strings on DONKI/NeoWs requests carry the NASA API key. Never let it
# reach logs: strip the query for the `url` field, and redact key/token/secret
# params from exception text (httpx error messages embed the full request URL).
_SECRET_QS_RE = re.compile(
    r"(?i)([?&](?:api_key|apikey|key|token|client_secret|secret)=)[^&\s'\"]+"
)
...
_GFZ_BASE = "https://kp.gfz.de"  # NOAA-independent Kp origin (fallback; gfz-potsdam.de 301s here)
...
async def _get_json(client: httpx.AsyncClient, url: str, **kwargs) -> Any:
    ...
    NOAA SWPC hands back HTTP 202 + an empty body when its cache/backend is
    regenerating or throttling (it can sit like that for hours). That is not an
    error, just "no data yet" — but blindly calling .json() on an empty body
    raised the cryptic 'Expecting value: line 1 column 1' spam. Treat an empty
    body as a clean miss and log it honestly instead.
    """
    try:
...
async def _fetch_kp_gfz(client: httpx.AsyncClient) -> tuple[float | None, str | None]:
    """NOAA-independent Kp fallback from GFZ Potsdam — the origin NOAA mirrors.
    Response shape: {"Kp": [floats, oldest-first], "datetime": [ISO strings]}.
    Returns (latest real Kp, time_tag) or (None, None). Kp is already in the
    standard 0-9 thirds scale (0, 0.333, 0.667, 1.0, ...), so _kp_level fits."""
    now = datetime.now(UTC)
    start = (now - timedelta(days=2)).strftime("%Y-%m-%dT%H:%M:%SZ")
    end = now.strftime("%Y-%m-%dT%H:%M:%SZ")
    data = await _get_json(
        client, f"{_GFZ_BASE}/app/json/?start={start}&end={end}&index=Kp",
    ...
    if not isinstance(vals, list):
        return None, None
    for i in range(len(vals) - 1, -1, -1):  # newest-first walk to the last real value
        try:
            kp = float(vals[i])
        except (ValueError, TypeError):
            continue
        tt = str(times[i]) if isinstance(times, list) and i < len(times) else None
        return kp, tt
    return None, None
...
async def fetch_kp(client: httpx.AsyncClient) -> dict:
    ...
    PRIMARY: the official 3-hour product `/products/noaa-planetary-k-index.json`
    (a list of dicts, oldest-first; key `Kp`) — the canonical index every
    space-weather app shows. We do NOT use the 1-min feed's integer `kp_index`
    as primary: it FLOORS to 0 in quiet periods (e.g. estimated_kp 0.33 →
    kp_index 0), which displayed a misleading Kp 0.0 while the real index sat
    at 1-2.
    FALLBACK: the 1-min `/json/planetary_k_index_1m.json` `estimated_kp` float.
    """
    ...
    # PRIMARY — official 3-hour planetary Kp (newest completed period = last row).
    prod = await _get_json(
        client, f"{_NOAA_BASE}/products/noaa-planetary-k-index.json"
    )
    if isinstance(prod, list) and prod and isinstance(prod[-1], dict):
        try:
            v = prod[-1].get("Kp")
            kp = float(v) if v is not None else None
            time_tag = str(prod[-1].get("time_tag"))
        except (ValueError, TypeError):
            kp = None

    # FALLBACK — 1-min ESTIMATED float (finer than the integer kp_index).
    if kp is None:
        data = await _get_json(
        ...
        if isinstance(data, list) and data and isinstance(data[-1], dict):
            ...
            if raw is None:
                raw = latest.get("kp_index")
            try:
                kp = float(raw) if raw is not None else None
                time_tag = str(latest.get("time_tag"))
            except (ValueError, TypeError):
                kp = None

    # FINAL FALLBACK — GFZ Potsdam, the definitive Kp origin NOAA republishes.
    # NOAA's whole SWPC service periodically returns 202/empty for hours at a
    # time; GFZ is an independent server, so Kp survives a NOAA blackout instead
    # of the vitals going blank.
    if kp is None:
        gkp, gtt = await _fetch_kp_gfz(client)
        if gkp is not None:
            kp, time_tag, source = gkp, gtt, "gfz"
...
async def fetch_solar_wind(client: httpx.AsyncClient) -> dict:
    """Latest solar-wind speed + Bz via NOAA SWPC's /products/summary/ single-value
    feeds. (NOAA retired the older /products/solar-wind/{plasma,mag}-*.json
    time-series ~mid-2026 → 404 spam; the summary feeds carry the latest speed/Bz
    we actually use — density/temperature are no longer served, and nothing renders
    them, so they degrade to None.)"""
    cached = tcache.get("solar_wind")
    if cached is not None:
        return cached
    speed = await _get_json(
        client, f"{_NOAA_BASE}/products/summary/solar-wind-speed.json"
    )
    mag = await _get_json(
        client, f"{_NOAA_BASE}/products/summary/solar-wind-mag-field.json"
    )
    out: dict = {"ok": False, "density_per_cm3": None, "temperature_k": None}

    def _row(d: Any) -> dict | None:
        # summary feeds return a single-element list wrapping one dict
        if isinstance(d, list) and d and isinstance(d[0], dict):
            return d[0]
        return d if isinstance(d, dict) else None

    if (srow := _row(speed)) is not None:
        try:
            sp = srow.get("proton_speed")
            out["speed_kms"] = float(sp) if sp not in (None, "") else None
            out["plasma_time_tag"] = str(srow.get("time_tag", ""))
            out["ok"] = True
        except (ValueError, TypeError):
            pass
    if (mrow := _row(mag)) is not None:
        try:
            bz, bt = mrow.get("bz_gsm"), mrow.get("bt")
            out["bz_nt"] = float(bz) if bz not in (None, "") else None
            out["bt_nt"] = float(bt) if bt not in (None, "") else None
            out["mag_time_tag"] = str(mrow.get("time_tag", ""))
            out["ok"] = True
        except (ValueError, TypeError):
            pass
...
async def fetch_xray(client: httpx.AsyncClient) -> dict:
    ...
    long_band = [
        d
        for d in data
        if isinstance(d, dict) and str(d.get("energy", "")).startswith("0.1-0.8")
    ]
    if not long_band:
        result = {"ok": False, "recent_flares_24h": []}
        tcache.put("xray", result)
...
async def fetch_neos(client: httpx.AsyncClient, days_ahead: int = 7) -> list[dict]:
    ...
    for date_str, objs in by_date.items():
        for obj in objs or []:
            # Per-record guard: one wrong-shaped NEO must not abort the whole feed
            # (this fetcher is also called directly by the get_near_earth_objects
            # tool, which doesn't go through snapshot_all's _safe wrapper).
            try:
...
async def fetch_earthquakes(
    ...
    out: list[dict] = []
    for f in features:
        # Per-record guard: one wrong-shaped feature (e.g. a string `mag`, or a
        # non-dict feature) must not abort the whole feed. This fetcher is also
        # called directly by get_earthquakes, bypassing snapshot_all's _safe.
        try:
            ...
            if mag is None:
                continue
            mag_f = float(mag)  # raises on non-numeric -> caught below
            # Filter NaN/inf too: float("nan") does NOT raise, and `nan < x` is
            # always False, so a NaN mag would otherwise slip through as a record.
            if not math.isfinite(mag_f) or mag_f < min_magnitude:
                continue
            out.append(
                {
                    "id": f.get("id"),
...
async def snapshot_all() -> dict:
    ...
    async def _safe(coro, fallback):
        # The contract above is "never raises". A single wrong-shaped NOAA/USGS
        # payload (which these endpoints HAVE returned — see the 202/empty-body
        # notes) must not crash the telemetry tool, the /api route, and the HUD
        # vitals block that all call this. Fail that one feed to its fallback.
        try:
            return await coro
        except Exception as e:  # noqa: BLE001 — deliberately broad; contract is no-raise
            log.warning("telemetry.snapshot_fetch_failed", error=_scrub_secrets(str(e)))
            return fallback

    async with httpx.AsyncClient() as client:
        kp = await _safe(fetch_kp(client), {"ok": False})
        sw = await _safe(fetch_solar_wind(client), {"ok": False})
        xray = await _safe(fetch_xray(client), {"ok": False, "recent_flares_24h": []})
        eonet = await _safe(fetch_eonet(client), [])
        neos = await _safe(fetch_neos(client), [])
        quakes = await _safe(
            fetch_earthquakes(client, period="day", min_magnitude=4.5), []
        )
