...
def _parse_polygon(poly: str | list[str] | None) -> list[tuple[float, float]]:
    """CAP polygon → [(lat, lon), ...].

    Accepts EITHER the legacy CAP-string form ``"lat,lon lat,lon ..."`` OR
    MeteoAlarm's current form (a JSON list of such strings, one per polygon
    ring). The list form started crashing this with
    ``'list' object has no attribute 'split'`` on every alert poll until this
    multi-shape parse landed. Distance computation only needs ALL vertices, so
    concatenating rings into one point set is correct.
    """
    if poly is None:
        return []
    rings = poly if isinstance(poly, (list, tuple)) else [poly]
    pts: list[tuple[float, float]] = []
    for ring in rings:
        for token in (str(ring) or "").split():
            if "," not in token:
                continue
            parts = token.split(",")
            try:
...
async def fetch_metoffice_warnings(
    lat: float, lon: float, radius_km: float, regions: str | list[str] | None = ""
) -> dict[str, Any]:
    ...
    # Accept either a comma-separated string (config-declared form) or a list
    # (pydantic-settings can parse a JSON-array env value as a list, which used
    # to crash with "'list' object has no attribute 'split'").
    if isinstance(regions, (list, tuple)):
        region_parts = [str(x) for x in regions]
    else:
        region_parts = (regions or "").split(",")
    region_set = {x.strip().lower() for x in region_parts if str(x).strip()}
