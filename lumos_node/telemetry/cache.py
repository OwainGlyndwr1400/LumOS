...
DEFAULT_TTL_SECONDS: dict[str, int] = {
    ...
    "gdacs": 900,         # GDACS RSS updates ~15 min
    "firms_raw": 1800,    # FIRMS NRT global CSVs refresh ~30 min; parse once
    "fires": 300,         # per-location filtered result — 5 min
    "satcat": 86400,      # CelesTrak SATCAT identity — slow-moving; 24 h
    "sar": 300,           # STAC scene search result — 5 min
    "outages": 600,       # IODA country outages — 10 min
    "schumann": 300,      # RHC — Schumann ELF amplitude; continuous, 5-min sample
}
...
# Hard bound on distinct cached sources. Keys can be parameterized by floats
# (lat/lon/elev), so without a cap the map would grow without limit for callers
# that vary those params. Over this, put() evicts expired then oldest-written.
...
def get(source: str) -> Any | None:
    ...
    if time.time() >= entry.expires_at:
        # Expired — evict the key so parameterized sources (lat/lon/elev encoded
        # in the key) can't accumulate forever. A later put() re-populates on a
        # real upstream hit anyway.
...
def put(source: str, value: Any, ttl_seconds: int | None = None) -> None:
    ...
    # Re-insert so dict order tracks write-recency (the size cap below evicts the
    # oldest-written first).
    _cache.pop(source, None)
    _cache[source] = _CacheEntry(value=value, expires_at=time.time() + ttl_seconds)
    _tick_counter(source)
    # Bound memory: over the cap, drop expired entries first, then evict the
    # oldest-written until back under the limit. Runs only when over the cap, so
    # the common path is untouched.
    if len(_cache) > _MAX_CACHE_ENTRIES:
        now = time.time()
        stale = [k for k, e in _cache.items() if now >= e.expires_at]
        for k in stale:
            _cache.pop(k, None)
...
DAILY_CAPS: dict[str, int | None] = {
    ...
    "gdacs": None,             # GDACS RSS: keyless, no documented cap
    "firms_raw": None,         # NASA FIRMS global CSV: keyless
    "fires": None,             # derived from firms_raw, no upstream of its own
    "satcat": None,            # CelesTrak: keyless, fair-use
    "sar": None,               # Element84/Copernicus STAC: keyless
    "outages": None,           # IODA: keyless
}
