...
def _write_state(settings: Settings, state: dict[str, Any]) -> None:
    ...
    try:
        # Atomic: write to a sibling temp then rename over the target, so a crash
        # mid-write can't leave a truncated state file (which the reader would
        # discard, wiping fires_today / dedup state).
...
def _append_log(settings: Settings, entry: dict[str, Any]) -> None:
    ...
    if n <= _LOG_CAP * 2:  # avoid pruning every write
        return
