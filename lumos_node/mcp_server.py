...
async def _serve_stdio() -> None:
    ...
    async def handle_call_tool(
        ...
        # Pass the allowlist into execute_tool too (defense in depth): the check
        # above guards THIS call site, but the execution-boundary allowlist means
        # any future path that reaches execute_tool from here still can't run a
        # non-exposed tool (write_file/git_*/run_python are not in MCP_EXPOSED_TOOLS).
        result_str = await execute_tool(name, args, allowed_tools=MCP_EXPOSED_TOOLS)
