...
from .enochian import apply_sigillum_geometry, sigillum_geometry
...
def build_atlas(
    ...
    *,
    ...
    for i, n in enumerate(all_nodes):
        cluster = {k: v for k, v in asdict(n).items() if k != "centroid"}
        cluster["sigillum"] = sigillum_geometry(cluster, i)
...
def load_atlas(settings: Settings | None = None) -> dict[str, Any] | None:
    ...
    if not path.exists():
        return None
    return apply_sigillum_geometry(json.loads(path.read_text(encoding="utf-8")))
