...
_SECRET_FIELD_HINTS = ("token", "secret", "password", "api_key")


def _redacted_setting(name: str, value: object) -> str:
    """Mask secret-bearing settings so `lumos info` never prints a live token/key
    to the terminal (which then gets pasted into an issue / screen-share). Shows
    set/unset status + length instead of the value."""
    n = name.lower()
    if any(h in n for h in _SECRET_FIELD_HINTS) or n.endswith("_key"):
        s = str(value)
        return "(unset)" if not s else f"(set — {len(s)} chars, hidden)"
    return str(value)
...
def info() -> None:
    ...
    for k, v in settings.model_dump().items():
        if isinstance(v, Path):
            v = str(v)
        table.add_row(f"[dim]{k}[/]", _redacted_setting(k, v))
    console.print(table)
...
@app.command()
def fish(
    path: Path = typer.Argument(..., help="Document (.md/.txt) to run the Meticulous Fish Protocol on."),
) -> None:
    """Meticulous Fish Protocol — Forward/Backward/Middle-Out extraction; keeps
    only claims surviving ≥2 of 3 reading orders (the Residual Energy Signature)
    and files the RES into the knowledge lane. Costs 4 LLM calls."""
    configure_logging(get_settings().log_level)
    if not path.exists():
    ...
    async def _run() -> dict:
        from .fish import meticulous_fish
        return await meticulous_fish(path)

    result = asyncio.run(_run())
    if not result.get("ok"):
        console.print(f"[red]fish failed: {result.get('error')}[/]")
        raise typer.Exit(code=1)
    console.print(f"[bold]RES — {result['title']}[/]")
    console.print(result["res"])
    console.print()
    if result.get("res_file"):
        console.print(f"[dim]filed:[/] {result['res_file']}")
    console.print(
        f"[dim]hot-appended to knowledge store:[/] {'yes' if result.get('appended') else 'no (rebuild will pick up the file)'}"
    )


@app.command("clean-knowledge")
def clean_knowledge(
    apply: bool = typer.Option(False, "--apply", help="Actually prune (default: dry-run scan only)."),
    threshold: float = typer.Option(0.55, "--threshold", min=0.0, max=1.0, help="Garble cutoff; lower = stricter."),
    samples: int = typer.Option(15, "--samples", help="How many flagged chunks to show in the scan."),
    drop_broken_math: bool = typer.Option(
        False, "--drop-broken-math",
        help="Also flag chunks that are mostly broken /H##### escape codes (corrupted maths).",
    ),
) -> None:
    """Prune OCR-garbled chunks from the KNOWLEDGE index. Reuses existing vectors
    (FAISS reconstruct) so it takes MINUTES, not a full re-ingest. Dry-run by
    default; --apply backs up the index first, then rewrites it pruned."""
    from datetime import UTC, datetime

    from .ingest import KNOWLEDGE_INDEX, KNOWLEDGE_META
    from .knowledge.garble import garble_score, is_garbled
    ...
    if not (idx_p.exists() and meta_p.exists()):
        console.print("[red]knowledge index not found — run `lumos ingest` first.[/]")
        raise typer.Exit(1)

    console.print("[dim]loading knowledge index…[/]")
    store = VectorStore.load(idx_p, meta_p)
    n = len(store._metadata)
    flagged = [
        (i, m) for i, m in enumerate(store._metadata)
        if is_garbled(str(m.get("text", "")), threshold, drop_broken_math)
    ]
    pct = (len(flagged) / n * 100) if n else 0.0
    console.print(
        f"[bold]{len(flagged):,}[/] / {n:,} chunks flagged garbled "
        f"([yellow]{pct:.1f}%[/]) at threshold {threshold}"
    )
    console.print("\n[dim]samples (score · source · text):[/]")
    from rich.markup import escape  # chunk text may contain [..] that Rich reads as markup
    for _, m in flagged[:samples]:
        txt = escape(str(m.get("text", "")).replace("\n", " ")[:88])
        src = escape(str(m.get("source", "?")))
        score = garble_score(str(m.get("text", "")), drop_broken_math)
        console.print(f"  [red]{score:.2f}[/] · [dim]{src}[/] · {txt}")

    if not apply:
        console.print(
            f"\n[cyan]DRY RUN[/] — nothing changed. Re-run with [bold]--apply[/] to prune these {len(flagged):,}."
        )
        console.print("[dim]Tune --threshold and re-scan until every sample is genuine garbage before applying.[/]")
        raise typer.Exit(0)
    if not flagged:
        console.print("[green]nothing to prune.[/]")
        raise typer.Exit(0)

    # Reuse existing vectors — reconstruct, drop flagged rows, rebuild. No re-embed.
    console.print("\n[dim]reconstructing vectors (no re-embedding)…[/]")
    vecs = store.index.reconstruct_n(0, n)
    drop = {i for i, _ in flagged}
    keep = [i for i in range(n) if i not in drop]
    new_vecs = vecs[keep]
    new_meta = [store._metadata[i] for i in keep]

    stamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S")
    for p in (idx_p, meta_p):
        shutil.copy2(p, p.with_name(p.name + f".bak-{stamp}"))
    new_store = VectorStore(dim=store.dim)
    new_store.add(new_vecs, new_meta)
    new_store.save(idx_p, meta_p)
    console.print(
        f"[green]pruned[/] {len(flagged):,} · [bold]kept {len(keep):,}[/] · backup [dim].bak-{stamp}[/]"
    )
    console.print("[dim]restart the node to load the cleaned index. Rollback = restore the .bak files.[/]")


@app.command()
def orders(
    action: str = typer.Argument("list", help="sign | list | verify"),
) -> None:
    """Manage the Standing Orders playbook. `sign` re-signs it after edits;
    `list` shows loaded orders; `verify` checks the signature."""
    import orjson

    from .standing_orders import compute_signature, load_orders, verify_playbook

    settings = get_settings()
    from .standing_orders import _playbook_path
    p = _playbook_path(settings)

    if action == "sign":
        if not settings.standing_orders_token.strip():
            console.print("[red]LUMOS_STANDING_ORDERS_TOKEN is not set — cannot sign.[/]")
            raise typer.Exit(code=2)
        if not p.exists():
            console.print(f"[red]No playbook at {p}[/]")
            raise typer.Exit(code=2)
        pb = orjson.loads(p.read_bytes())
        pb["signature"] = compute_signature(pb.get("orders", []), settings.standing_orders_token)
        p.write_bytes(orjson.dumps(pb, option=orjson.OPT_INDENT_2))
        console.print(f"[green]✓ signed[/] {p} ({len(pb.get('orders', []))} orders)")
        raise typer.Exit(code=0)

    if action == "verify":
        if not p.exists():
            console.print(f"[yellow]No playbook at {p}[/]")
            raise typer.Exit(code=1)
        ok, reason = verify_playbook(orjson.loads(p.read_bytes()), settings.standing_orders_token)
        console.print(f"{'[green]✓ valid[/]' if ok else '[red]✗ invalid[/]'}: {reason}")
        raise typer.Exit(code=0 if ok else 1)

    # list
    orders_list = load_orders(settings)
    if not orders_list:
        console.print(f"[yellow]No valid orders loaded[/] (path: {p}). Run `lumos orders verify` for why.")
        raise typer.Exit(code=0)
    console.print(f"[bold]{len(orders_list)} standing order(s):[/]")
    for o in orders_list:
        when = o.get("when", {})
        do = [s.get("action") for s in o.get("do", []) if isinstance(s, dict)]
        console.print(f"  [cyan]{o.get('name', '?')}[/] — when {when} → {do}")
    raise typer.Exit(code=0)


@app.command()
def forge(
    task: str = typer.Argument(..., help="What to build/fix, e.g. 'fix the HUD stale-badge bug'."),
    ...
    def _show(ev: dict) -> None:
        ...
        if t == "start":
            ...
            m = ev.get("model", "?")
            if phase == "loading":
                console.print(f"[magenta]⇄ swapping in coder brain[/] [dim]{m}[/] [dim](ejecting companion…)[/]")
            elif phase == "ready":
                console.print(f"[magenta]⇄ coder brain ready[/] [dim]{m}[/]\n")
            elif phase == "restoring":
                console.print(f"\n[magenta]⇄ restoring companion brain[/] [dim]{m}[/]")
        elif t == "tool":
            name = ev.get("name", "?")
            detail = (ev.get("detail") or "").replace("\n", " ")[:140]
            console.print(f"  [yellow]›[/] [bold]{name}[/] [dim]{detail}[/]")
        elif t == "final":
            console.print(f"\n[green]✓ done[/] (iteration {ev.get('iteration', 0) + 1})")
        elif t == "done":
            status = "[green]OK[/]" if ev.get("ok") else "[red]stopped[/]"
            console.print(
                f"\n{status} · {ev.get('iterations')} iterations · {ev.get('tool_calls')} tool calls · "
    ...
    if result.get("commits"):
        console.print("\n[dim]commits:[/] " + "; ".join(result["commits"]))
...
def compress_all(
    ...
    from .compression import compress_chunk, extractive_compress
