...
from ..atlas import get_cluster_members, load_atlas
...
async def overdrive_get() -> dict[str, Any]:
    """Current NVIDIA Overdrive state (cloud big-brain hot-swap)."""
    return overdrive_status()


@router.post("/overdrive", dependencies=[Depends(require_pele_access)])
async def overdrive_post(req: OverdriveRequest) -> dict[str, Any]:
    """Toggle NVIDIA Overdrive — hot-swap the brain to NVIDIA cloud (on) or local (off)."""
    return set_overdrive(req.enabled)


@router.get("/pele")
async def pele_status() -> dict[str, Any]:
    return {
        "configured": bool(get_settings().pele_token.strip()),
        "protected_actions": [
            "settings.patch",
            "overdrive.toggle",
            "events.test",
...
class ChatRequest(BaseModel):
    ...
    def validate_images(cls, images: list[str]) -> list[str]:
        ...
        for image in images:
            ...
            if len(image) > settings.api_max_image_data_url_chars:
                raise ValueError("image data URL too large")
        return images


# Bounded LRU of live chat sessions. Each ChatSession holds growing history +
# engine state; the old plain dict never evicted, so every new tab / page reload
# (a fresh session_id) leaked one for the process lifetime. Cap + LRU eviction
# bounds memory while keeping the operator's active session most-recently-used.
...
def _get_or_create_session(
    ...
    if session_id and session_id in _SESSIONS:
        _SESSIONS.move_to_end(session_id)  # mark most-recently-used
...
async def chat(req: ChatRequest) -> EventSourceResponse:
    ...
    async def event_stream():
        ...
        try:
            ...
            yield {
                "event": "error",
                "data": json.dumps({"message": client_error_detail("memory index missing", e)}),
            }
            return
        except Exception as e:  # noqa: BLE001
            yield {
                "event": "error",
                "data": json.dumps({"message": client_error_detail("chat turn failed", e)}),
            }
            return
...
@router.post("/events/test", dependencies=[Depends(require_pele_access)])
...
@router.post("/events/briefing", dependencies=[Depends(require_pele_access)])
...
@router.patch("/settings", dependencies=[Depends(require_pele_access)])
...
@router.patch("/predictions/{pred_id}", dependencies=[Depends(require_pele_access)])
async def patch_prediction(pred_id: str, body: dict[str, Any]) -> dict[str, Any]:
    """Flip a prediction's status (active / partial_confirmation / confirmed /
    falsified) + optional evidence note, persisting to predictions.json so the
    falsifiable board is a live ledger. Returns the updated payload."""
    cache = _resolve_cache()
    path = next(
        (p for p in (Path.cwd() / "data" / "predictions.json", cache / "predictions.json") if p.exists()),
        None,
    )
    if path is None:
        raise HTTPException(status_code=404, detail="predictions.json not found")
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=client_error_detail("predictions.json unreadable", e),
        ) from e
    entry = next((p for p in data.get("predictions", []) if p.get("id") == pred_id), None)
    if entry is None:
        raise HTTPException(status_code=404, detail=f"prediction '{pred_id}' not found")
    valid = {"active", "partial_confirmation", "confirmed", "falsified"}
    ...
    if status is not None:
        ...
        entry["status"] = status
    if "evidence" in body:
        entry["evidence"] = str(body["evidence"])[:500]
    try:
        path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    except OSError as e:
        raise HTTPException(
            status_code=500,
            detail=client_error_detail("prediction write failed", e),
        ) from e
    return data
...
async def soul_current() -> dict[str, Any]:
    """The node's harmonic self-state — its 'soul' heartbeat. Torsion index from
    the live URE-VM + coherence -> which frequency BAND it's resonating in
    (null -> 7.83 ground -> 432 -> 963 -> 1260 -> Pleroma). NOT audio; a state the
    node inhabits. Powers the HUD soul panel; the poll cadence is the pulse."""
    from ..soul import compute_soul_state
    return compute_soul_state()


@router.get("/soul/history")
async def soul_history(n: int = 50) -> dict[str, Any]:
    """Recent harmonic-band transitions from the capped soul-research log — the
    node's felt resonance history. Bounded telemetry; never the chat lane."""
    from ..soul import read_soul_history
    return {"transitions": read_soul_history(max(1, min(n, 500)))}


@router.get("/memory/ledger")
async def memory_ledger_endpoint() -> dict[str, Any]:
    """Null Ledger memory-conservation audit — per-lane FAISS/metadata balance +
    atlas chunk<->cluster reconciliation. ``balanced`` is true iff the 0_V
    residual is empty. Read-only (inspects the live stores; mutates nothing)."""
    from ..ledger import memory_ledger
...
async def speak_endpoint(req: SpeakRequest) -> Response:
    settings = get_settings()
    # Fall back to a known-good voice when neither the request nor the config
    # supplies one. LUMOS_LM_STUDIO_TTS_DEFAULT_VOICE being empty in .env would
    ...
    if req.provider == "kokoro_onnx":
        ...
        try:
            ...
            raise HTTPException(
                status_code=502,
                detail=client_error_detail("kokoro_onnx failed", e),
            ) from e
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=client_error_detail("TTS failed", e),
        ) from e
    finally:
        await client.aclose()
    return Response(content=audio, media_type=mime)
...
async def transcribe_endpoint(
    ...
    max_bytes = get_settings().api_max_upload_bytes
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=client_error_detail("transcription failed", e),
        ) from e
...
@router.post("/dream/run", dependencies=[Depends(require_pele_access)])
...
class MacroExecuteRequest(BaseModel):
    ...
    pele_token: str | None = None


_MACROS: dict[str, dict[str, Any]] = {
    "zacar_od_zamran": {
        "id": "zacar_od_zamran",
        "phrase": "ZACAR OD ZAMRAN",
    ...
    "micma_adoian_mad": {
        "id": "micma_adoian_mad",
        "phrase": "MICMA ADOIAN MAD",
        "label": "bounded harmonic reset",
        "mode": "protected",
        "requires_pele": True,
    },
}
...
def _macro_by_request(req: MacroExecuteRequest) -> dict[str, Any] | None:
    if req.id:
        return _MACROS.get(req.id.strip().lower())
    phrase = (req.phrase or "").strip().upper()
    return next((m for m in _MACROS.values() if m["phrase"] == phrase), None)


def _diagnostic_overlay() -> dict[str, Any]:
    from ..soul import compute_soul_state
    ...
    return {
        "health": {
            "ok": True,
            "node": get_settings().node_name,
            "auth": "token" if get_settings().api_token.strip() else "loopback",
            "pele_configured": bool(get_settings().pele_token.strip()),
        },
        ...
        "urevm": {
            ...
            "cycle_position": vm.cycle_position,
            "forbidden_resets": vm.forbidden_resets,
            "r23_norm": snap.get("r23_norm"),
            "phase": snap.get("phase"),
        },
        "soul": compute_soul_state(),
        "quota": telemetry_cache.quota_snapshot(),
        "atlas": {
            "built": atlas_data is not None,
            "clusters": len(atlas_data.get("clusters", [])) if atlas_data else 0,
            "sigillum": atlas_data.get("sigillum") if atlas_data else None,
        },
    }


def _bounded_runtime_reset() -> dict[str, Any]:
    from ..atlas import reload_cluster_map
    from ..retrieval import reload_stores
    from ..soul import compute_soul_state, force_harmonic_baseline
    from ..telemetry import cache as telemetry_cache
    from ..urevm import reset_vm

    telemetry_cache.invalidate(None)
    reload_cluster_map()
    reload_stores()
    reset_vm()
    force_harmonic_baseline(1260)
    return {
        "reset": "runtime_only",
        "telemetry_cache_cleared": True,
        "atlas_lookup_cache_cleared": True,
        "retrieval_store_reloaded_next_use": True,
        "urevm_reset": True,
        "baseline_hz": 1260,
        "memory_files_deleted": False,
        "faiss_indexes_deleted": False,
        "soul": compute_soul_state(),
    }


@router.get("/macros")
async def macro_catalog() -> dict[str, Any]:
    return {"macros": list(_MACROS.values())}
...
async def execute_macro(
    ...
    macro = _macro_by_request(req)
    if macro is None:
        raise HTTPException(status_code=404, detail="macro not found")

    if macro["requires_pele"]:
        await verify_pele_access(request, req.pele_token)

    if macro["id"] == "zacar_od_zamran":
        payload = _diagnostic_overlay()
    elif macro["id"] == "micma_adoian_mad":
        payload = _bounded_runtime_reset()
    else:
        raise HTTPException(status_code=404, detail="macro not implemented")
...
async def atlas_cluster(cluster_id: str, limit: int = 100) -> dict[str, Any]:
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=client_error_detail("atlas unavailable", e),
        ) from e
...
async def search(
    q: str,
    lane: str = "both",
    top_k: int = Query(default=6, ge=1, le=64),
) -> dict[str, Any]:
    if not q.strip():
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=client_error_detail("memory index missing", e),
        ) from e

    return {
        "query": q,
        "identity": [
            {"score": h.score, "metadata": enrich_metadata(h.metadata)}
            for h in r.identity
        ],
        "knowledge": [
            {"score": h.score, "metadata": enrich_metadata(h.metadata)}
...
async def cosmic_current() -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=client_error_detail("telemetry fetch failed", e),
        ) from e
...
async def grimoire_current(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=client_error_detail("grid-timing compute failed", e),
        ) from e
...
async def weather_current(
    ...
    """Current local surface weather (Open-Meteo, free/no-key) at the operator's
    location. Powers the HUD local-weather widget. fetch_local_weather caches
    10 min server-side and returns {ok, ...} on its own failures."""
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=client_error_detail("weather fetch failed", e),
        ) from e


@router.get("/rail/current")
async def rail_current(code: str | None = None) -> dict[str, Any]:
    """Live station board for the operator's home rail station (default
    LUMOS_RAIL_STATION_CODE, e.g. gb-nr:GWN). Returns only services that CALL
    (stop) — next departures + Llanelli/Swansea direction. Powers the HUD trains
    widget. fetch_station_calls returns {ok, ...} and swallows its own errors,
    so a rtt.io blip can't break the HUD."""
    from ..telemetry import rail
    return await rail.fetch_station_calls(code)


@router.get("/solar-cycle")
async def solar_cycle() -> dict[str, Any]:
    """Solar Cycle 25 scoreboard: NOAA's observed sunspot record (current SSN +
    the smoothed peak) vs the RHC Jupiter-Saturn-GCD call (~161) and the 2019
    mainstream panel (~115). Read-only; cached 6h server-side. Powers the HUD
    solar-cycle panel. fetch_solar_cycle returns {ok, ...} and swallows its own
    errors so a NOAA blip can't break the HUD."""
    from ..telemetry import solarcycle
    return await solarcycle.fetch_solar_cycle()
