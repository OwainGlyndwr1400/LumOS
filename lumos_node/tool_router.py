...
TOOL_CATEGORIES: dict[str, list[str]] = {
    ...
    # Phase 44 — GeoSentinel expansion (all read-only/passive-safe).
    "disaster": ["disaster_alerts", "fires_nearby"],
    "conflict_zones": ["conflict_zones_nearby"],
    "outages": ["internet_outages"],
    "imagery": ["satellite_imagery_coverage"],
    # Batch 2 — satellites.
    ...
    # get_weather (current local conditions, Open-Meteo) rides the same category:
    # both are passive, location-defaulted, and surfaced in the dawn briefing.
    "forecast": ["get_forecast", "get_weather", "get_weather_warnings"],
...
# categories: telemetry (sensing) + memory (recall) + time. Erydir's locked
...
AUTONOMOUS_PASSIVE_CATEGORIES: frozenset[str] = frozenset(
    {
        ...
        # Phase 44 GeoSentinel — all read-only sensing, safe for autonomous wakes.
        "disaster", "conflict_zones", "outages", "imagery",
    }
)
...
CATEGORY_TRIGGERS: dict[str, tuple[str, ...]] = {
    ...
    # Phase 44 — GeoSentinel expansion.
    "disaster": (
        "disaster", "disasters", "gdacs", "hazard", "hazards", "cyclone",
        "hurricane", "typhoon", "flood", "flooding", "volcano", "eruption",
        "tsunami", "drought", "fire", "fires", "wildfire", "wildfires",
        "hotspot", "burning", "any fires",
    ),
    "conflict_zones": (
        "conflict zone", "conflict zones", "war zone", "war zones", "active conflict",
        "which conflicts", "where is the fighting", "theatre", "theatres",
        "nearest conflict", "hotspots", "flashpoint",
    ),
    "outages": (
        "internet outage", "internet outages", "outage", "outages", "blackout",
        "connectivity", "internet shutdown", "gone offline", "went offline",
        "internet down", "ioda",
    ),
    "imagery": (
        "satellite imagery", "sar", "radar imagery", "sentinel", "imaged",
        "imagery coverage", "last imaged", "satellite photo", "earth observation image",
    ),
    "satellites": (
        "satellite", "satellites", "spacecraft", "overhead pass", "iss",
        "starlink", "orbit", "what's in space", "space station", "sat pass",
        "what's overhead", "whats overhead", "above me", "over my head",
    ),
    "maritime": (
    ...
    "forecast": (
        ...
        # current local surface weather (get_weather, Open-Meteo)
        "weather", "temperature", "how hot", "how cold", "how warm", "degrees",
        "wind", "windy", "rain", "raining", "is it raining", "humidity",
        "outside", "how's it outside", "hows it outside", "what's it like out",
        # severe-weather warnings (get_weather_warnings, MeteoAlarm + Open-Meteo)
        "warning", "warnings", "severe weather", "storm", "storms", "gale",
        "amber alert", "red alert", "yellow warning", "met office", "flood",
        "thunderstorm", "extreme weather", "weather warning",
    ),
    "watches": (
