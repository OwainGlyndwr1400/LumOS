...
def _is_within(path: Path, root: Path) -> bool:
    """True if `path` is inside `root` (both already resolved)."""
    try:
...
@register(
    name="write_file",
    description=(
        "WRITE TEXT CONTENT to a file. CALL THIS whenever Erydir asks you to save, "
...
def append_file(path: str, content: str) -> dict:
    ...
    if len(encoded) > 5_000_000:
        return {"error": f"content too large: {len(encoded):,} bytes (max 5MB)"}
    # Cumulative cap: each CALL was capped at 5MB, but nothing bounded the file's
    # TOTAL size across many appends — it could grow without limit (and a later
    # read_file would try to load it all). Reject once it would exceed the cap.
    existing = target.stat().st_size if target.exists() else 0
    if existing + len(encoded) > 50_000_000:
        return {
            "error": f"file too large: appending would exceed 50MB (current {existing:,} bytes)"
        }
    try:
        target.parent.mkdir(parents=True, exist_ok=True)
...
def list_allowed_paths() -> dict:
    ...
    return {
        ...
        "count": len(allowed),
        "hint": (
            "No paths configured. Tell Erydir to set LUMOS_TOOL_ALLOWED_PATHS."
            if not allowed
...
@register(
    name="read_file",
    description=(
        "Read the contents of a text file from disk. CALL THIS whenever Erydir "
        ...
        "them directly. Returns up to 80KB of text. Useful for Erydir's research "
...
def read_file(path: str) -> dict:
    ...
    # Bounded read: pull at most the cap off disk instead of loading the whole
    # file into memory and then slicing (a multi-GB file would OOM the process
    # before the 80KB truncate ever ran). read(N) on a text handle is N chars.
    read_cap = 80_000
    try:
...
def list_files(path: str, pattern: str = "*") -> dict:
    ...
    # The `pattern` argument also needs sandboxing: Path.glob honors '..', so a
    # pattern like '../secret/*' escapes the validated `target` even though
    # `path` itself was checked. Reject upward/absolute patterns outright, and
    # defensively drop any hit that resolves outside the allowed roots (this also
    # catches symlinks pointing out of the sandbox). Cap enumeration cost too:
    # the old sorted(glob)[:200] materialized every match before truncating.
    pat = pattern.strip() or "*"
    if ".." in Path(pat).parts or pat.startswith(("/", "\\")) or (len(pat) >= 2 and pat[1] == ":"):
    ...
    try:
        for p in target.glob(pat):
            ...
            if not any(_is_within(resolved, a) for a in allowed):
                continue  # escaped the sandbox (traversal or symlink) — skip
            items.append(
                {
                    "name": p.name,
                    "path": str(p),
    ...
    return {
        ...
        "count": len(items),
        "items": items,
        "truncated": scanned > scan_cap or len(items) >= hit_cap,
    }
