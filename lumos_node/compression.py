...
- the essential meaning and continuity; discard noise and filler
...
# Hard ceiling on a single compression generation. A 3-layer packet is small, so
# this cap — together with enable_thinking=False on the call — structurally
# prevents the model running away in a degenerate reasoning loop (observed:
# gpt-oss-20b repeating "Lost-2"/"core truth"/"binding-energy" for 1000+ reasoning
# tokens with no terminator). Compression is opt-in metadata; a cut-off response
# just falls back to full text, never blocks dream consolidation.
COMPRESSION_MAX_TOKENS = 700

# Templated alert traffic (autonomous wakes / dawn briefings) is ALREADY a
# summary — asking the LLM to re-summarize it is where gpt-oss-20b degenerated
# into ellipsis spam ("…","…","…" inside the schema until the length cap;
# observed 2026-06-11 on a backlog of train-alert turns). These chunks route
# straight to extractive compression: better output, zero LLM time.
_TELEMETRY_MARKERS = ("[AUTONOMOUS WAKE", "[DAWN BRIEFING")

# Degenerate-output detectors: a run of 15+ of the same character, dense
# ellipsis/dot/slash runs, or a tail that is mostly punctuation. The "…"
# (U+2026) spam loop hits all three.
_DEGENERATE_RUN_RE = re.compile(r"(.)\1{14,}")
_PUNCT_SPAM_RE = re.compile(r"[…./\\,?\-]{12,}")


def _looks_degenerate(s: str) -> bool:
    """True when a generation has collapsed into repetition/punctuation spam."""
    if _DEGENERATE_RUN_RE.search(s) or _PUNCT_SPAM_RE.search(s):
        return True
    tail = s[len(s) // 2:]
    if len(tail) > 200:
        alnum = sum(1 for ch in tail if ch.isalnum())
        if alnum / len(tail) < 0.35:
            return True
    return False
...
async def compress_chunk(
    ...
    # gpt-oss spec sampling (temp 1.0 / top_p 1.0). The previous 0.3 here —
    # and off-spec UI presets (0.7/0.8) — narrow the distribution enough that
    # a single filler token ("…") can dominate and self-reinforce under the
    # JSON grammar. Output structure is constrained by the schema anyway.
    ...
    # Alert-traffic short-circuit: wake/briefing turns are templated summaries
    # already — extractive keeps the entities/times perfectly and costs nothing.
    if any(marker in text for marker in _TELEMETRY_MARKERS):
        log.info("compression.telemetry_turn_extractive")
    ...
    try:
        ...
        response = await client.chat(
            ...
            # Compression is mechanical — thinking mode adds no value here and is
            # exactly where gpt-oss-20b degenerated into the repetition loop.
            chat_template_kwargs={"enable_thinking": False},
        )
        content = response.get("content") or ""
        if not content.strip():
            log.info("compression.empty_response_extractive_fallback")
            return extractive_compress(text)

        # Degeneracy guard BEFORE parsing: a generation that hit the length cap
        # or collapsed into repetition spam is garbage even if it parses.
        if response.get("finish_reason") == "length" or _looks_degenerate(content):
            log.info(
                "compression.degenerate_output_extractive_fallback",
                finish_reason=response.get("finish_reason"),
            )
            return extractive_compress(text)

        parsed = json.loads(content)
        # Post-parse sanity: drop key_points that are punctuation husks; if the
        # list collapses, the whole packet is suspect — extractive instead.
        so = parsed.get("summary_object") or {}
        kps = so.get("key_points")
        if isinstance(kps, list):
            cleaned = [
                kp for kp in kps
                if isinstance(kp, str) and sum(1 for ch in kp if ch.isalnum()) >= 3
            ]
            if kps and not cleaned:
                log.info("compression.husk_key_points_extractive_fallback")
                return extractive_compress(text)
            so["key_points"] = cleaned
        ...
        # gpt-oss-20b occasionally emits truncated/invalid JSON under the strict
        # schema — fall back to the no-LLM heuristic so the chunk STILL compresses
        # (same output shape, never raises) instead of being skipped.
        log.info("compression.json_decode_failed_extractive_fallback", error=str(e))
        return extractive_compress(text)
    except Exception as e:  # noqa: BLE001
        log.info("compression.failed_extractive_fallback", error=str(e))
        return extractive_compress(text)
    finally:
