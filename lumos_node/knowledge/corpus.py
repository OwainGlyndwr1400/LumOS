...
def _iter_text_blocks(path: Path, max_chars: int) -> Iterator[tuple[str, str, str]]:
    """Paragraph-accumulated (subject, text, seed) blocks — the chunk-type-
    agnostic core shared by the knowledge-corpus and identity-corpus loaders.
    Splits on blank lines, packs paragraphs up to `max_chars`."""
    try:
    ...
    for p in paragraphs:
        if size + len(p) > max_chars and buf:
            idx += 1
            text = "\n\n".join(buf)
            yield (f"{title} §{idx}", text, buf[0][:200])
            buf, size = [], 0
        buf.append(p)
        size += len(p)
    if buf:
...
def _iter_text_chunks(path: Path) -> Iterator[KnowledgeChunk]:
    """Paragraph-accumulated KNOWLEDGE chunks for prose files (blueprints, notes)."""
    for subject, text, seed in _iter_text_blocks(path, _TEXT_MAX_CHARS):
        yield _make_chunk(path.name, subject, text, seed)
...
# ── Identity corpus (dream-codex / journals → the "who Lumos is" lane) ────────
# Narrative prose that belongs with the identity conversations rather than the
# knowledge lookup lane (e.g. gnostic-engine dream pings). Same paragraph
# chunker, but wrapped as IdentityChunk and tagged so the composer/HUD render
# them like any identity memory. Chunked at the identity lane's own ~2000-char
# scale (vs knowledge's 1200) so a narrative dream stays coherent per chunk.
_IDENTITY_TEXT_MAX_CHARS = 2000


def iter_identity_corpus_chunks(extra_dir: Path):
    """Prose files under extra_dir → IdentityChunk stream.

    .md / .txt only — a CSV theorem table is structured *knowledge*, not
    identity, so it's skipped here (it belongs in LUMOS_KNOWLEDGE_EXTRA_DIR).
    Tagged conversation_id='dreamcodex:<file>', roles=['dream']; file mtime
    stands in as the chunk timestamp so dreams carry a rough 'when'.
    Return type is IdentityChunk (imported lazily to avoid a package cycle).
    """
    ...
    for path in corpus_files(extra_dir):
        ...
        try:
            ...
            mtime = None
        for subject, text, _seed in _iter_text_blocks(path, _IDENTITY_TEXT_MAX_CHARS):
            cid = _chunk_id(path.name, text)
            yield IdentityChunk(
                chunk_id=cid,
                conversation_id=f"dreamcodex:{path.stem}",
                conversation_title=subject,
                create_time_first=mtime,
                create_time_last=mtime,
                roles=["dream"],
                node_ids=[cid],
                text=text,
            )
