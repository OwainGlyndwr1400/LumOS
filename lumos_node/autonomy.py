...
# Cap the in-process history. CRITICAL: bound it by SIZE, not just message count.
# A single rich wake is now ~4k+ tokens, so the old 20-MESSAGE cap let ~10 prior
# mega-pings stack into a ~52k-token prompt AND fed the model its own prior
# breakdowns, which it escalated ("8th COSMOS… 31 pings total"). We keep a short
# rolling window AND truncate each remembered ping to a snippet: enough for
# "I flagged that ship earlier" continuity, without the bloat or the self-
# amplifying loop. Full pings still persist to identity_events.jsonl and are
# recoverable via retrieval.
_MAX_HISTORY_MESSAGES = 12       # last ~6 wake pairs
_REMEMBERED_PING_CHARS = 700     # store only the head of each prior ping
...
def get_autonomous_session() -> ChatSession:
    ...
    # Truncate remembered ASSISTANT pings to a snippet so one 4k-token breakdown
    # can't blow the next prompt to 52k or seed an escalating narrative.
    for i, m in enumerate(sess.history):
        if (
            m.role == "assistant"
            and isinstance(m.content, str)
            and len(m.content) > _REMEMBERED_PING_CHARS
        ):
            sess.history[i] = ChatMessage(
                role="assistant",
                content=m.content[:_REMEMBERED_PING_CHARS] + " […]",
            )
    return sess
...
async def trigger_dawn_briefing(bus: EventBus | None = None) -> str:
    ...
    trigger = {
        "mode": "briefing",
        "kinds": ["dawn_briefing"],
        "summary": "Dawn briefing — Erydir just woke; the night behind and the day ahead.",
        "events": events,
    }
