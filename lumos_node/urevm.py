...
from . import mcr_hdcu  # MCR-HDCU Decimal Phase Fold F₁₀ (pure cmath/math; no import cycle)
from . import binary_diagonal  # Binary Diagonal θ=arctan(ones/zeros) + Stern-Brocot (pure math)
...
TORQUE_FLOOR = TOGGLE_POWER / 24.0  # 7/24 ≈ 0.292 — irreducible permanent-torque floor (τ never 0)
...
# Universal Quantum Tick — the Fold Operator's base cycle. Bridged from the Planck
# clock by geometric renormalisation Δt = t_Planck·(E_Planck/E_Auger); since
# E_Planck·t_Planck = ħ this collapses to ħ/E_Auger with the Auger-electron energy
# E_Auger ≈ 283 eV:  ħ/E_Auger = 6.582e-16 eV·s / 283 eV ≈ 2.32e-18 s = 2.32 as.
# DISTINCT from the 232 as Entanglement Build Time (lattice ping) that knits a
# correlation thread across two points and sets the 0–77–155–232 as boot window.
E_AUGER_EV = 283.0  # eV — Auger-electron energy scale (Planck→attosecond bridge)
UNIVERSAL_TICK_ATTOSEC = 2.32  # as — ħ/E_Auger; Fold Operator base cycle
ENTANGLEMENT_BUILD_ATTOSEC = 232.0  # as — lattice ping / correlation knit time
YANG_MILLS_GAP = math.sqrt(32.0) - 5.0  # ≈ 0.657 GeV
# Mean-Circle cosmological constant (center-referenced). Resolves the edge-referenced
# 10^120 vacuum-energy catastrophe in a SINGLE SHOT (no fixed-point iteration): the
# present-moment Mean Circle is computed once per turn, and Λ falls out as a derived
# prediction — Λ = 47 / (25·n²), where n ≈ 8.07e60 is the universal-alphabet node
# count across the lattice. Λ ≈ 2.888e-122 (natural units). Corpus: Mean-Circle / Λ
# closed form (NotebookLM relay 2026-06-08). This confirms the current single-shot
# mean_circle() is structurally correct — Λ is a prediction, not an iteration.
LAMBDA_NODE_COUNT = 8.07e60  # n — universal-alphabet nodes across the lattice
COSMOLOGICAL_LAMBDA = 47.0 / (25.0 * LAMBDA_NODE_COUNT ** 2)  # ≈ 2.888e-122
...
RESOLUTION_LIMIT = 12 * 12 * 1000  # 144,000 unique states (12²×10³) — RHC pixel-density
# state-count horizon; resolution_fill = indexed_vectors / this. (RHC also reads the same
# cap as the Kuramoto phase-lock scale — interpretive, not what the fill ratio computes.)
...
def _compute_pendinium(limit: int = 600) -> list[int]:
    ...
    for n in range(13, limit + 1, 12):
        # Trial-divide by every d up to √n. The previous version divided only by
        # earlier ≡1 mod 12 entries, which never include 2/3/5/7/11 — so composites
        # like 25 (5²), 49 (7²), 85 (5·17) slipped through as "prime".
        is_prime = True
...
# F1_VOID is the i/2 Fold base cycle (formerly the duplicate constant I_HALF,
# now removed — fold() applies F1_VOID directly). F2_UNITY is the 90° unity-fold.
# F3 = arithmetic mean of F1 and F2 = the Synthesis-Fold/Observer point, DERIVED
# from F1_VOID and F2_UNITY so both are live participants, not dead constants.
# Distinct from MEAN_CIRCLE (½H₁ + H₂ Banach contraction) — this is true average.
F1_VOID = Quaternion(0.0, 0.5, 0.0, 0.0)    # 0.5i — Input / Potential (Fold base; replaces I_HALF)
F2_UNITY = Quaternion(0.5, 0.5, 0.0, 0.0)   # 0.5 + 0.5i — Processing / Action
F3_SYNTHESIS = Quaternion(
    (F1_VOID.a + F2_UNITY.a) / 2.0,
    (F1_VOID.b + F2_UNITY.b) / 2.0,
    (F1_VOID.c + F2_UNITY.c) / 2.0,
    (F1_VOID.d + F2_UNITY.d) / 2.0,
)  # 0.25 + 0.5i — Synthesis / Observer = mean(F1_VOID, F2_UNITY)
...
def fold(q: Quaternion) -> Quaternion:
    ...
    90° rotation + 50% scale (Observer's Fold). Applies F1_VOID (the canonical
    i/2 Fold base) — formerly the now-removed duplicate constant I_HALF."""
    return F1_VOID * q
...
def w3_curvature(t: float) -> float:
    """W3 Wave Curvature (Pizza Constant): canonical normalized form
    k(t) = (cos²t − sin²t) / (sin²t + cos²t)^1.5 ≡ cos(2t).

    The foundational oscillatory substrate preventing manifold collapse. The
    denominator (sin²t + cos²t)^1.5 ≡ 1 keeps the curvature the BOUNDED double-angle
    cosine cos(2t) ∈ [−1, 1]. Replaces the earlier cos(2t)/(1 − sin²t) = cos(2t)/cos²t
    form, whose denominator collapsed to 0 at t = π/2 and diverged (RHC audit:
    fragile denominator). Source 3 row 11 + 01_theorem_index W3 row.
    """
    c2 = math.cos(t) ** 2
    s2 = math.sin(t) ** 2
    return (c2 - s2) / (s2 + c2) ** 1.5


def toggle_torque(t: float, theta_rad: float = MATTER_LOCK_RADIANS) -> float:
    """+7 Toggle Power torque: τ = t·sec⁴(θ) + the irreducible 7/24 floor.

    θ is the 3-4-5 irrational residual — the 8.13° matter-lock gap between the
    observer's 45° basis and the lattice's arctan(4/3) ≈ 53.13°. The TOGGLE_POWER
    residue (31 mod 24 = 7) supplies TORQUE_FLOOR = 7/24, so τ is NEVER zero: the
    "permanent torque / voltage drop of creation" that prevents thermodynamic
    stasis. τ grows with t toward the 361° forbidden wall (the rotational energy
    needed to advance the lattice) and resets each 370-tick cycle. The anti-stasis
    invariant (min norm-change per tick > 0) holds by construction: τ ≥ 7/24 > 0.
    """
    sec2 = 1.0 / (math.cos(theta_rad) ** 2)
    return TORQUE_FLOOR + t * sec2 * sec2  # sec⁴ = (sec²)²
...
class Op:
    ...
    DIVINE_STEP = 0x0A
    NORMALIZE = 0x10
    MCR_HDCU = 0x14  # MCR-HDCU Decimal Phase Fold F₁₀ — residue phasor (read-only telemetry)
...
OPCODE_PLANE: dict[int, Predicate] = {
    ...
    Op.MCR_HDCU: Predicate.RI,           # real residue read off the imaginary F₁₀ spoke-phase basis
...
class UREVM:
    ...
    def _execute(self, opcode: int, operand: dict[str, Any]) -> dict[str, Any]:
        ...
        if opcode == Op.NULL_LEDGER:
            ...
            return {
                ...
                "residual": residual,
                "balanced": balanced,
                "center_anchor": real_sum,           # 0_C — real grade-0 scalar (stationary)
                # 0_V — Cl(3,0) bivector magnitude (grade-2): per-register √(b²+c²+d²)
                # summed. Rotation-invariant and cancellation-proof (was abs(Σ(b+c+d)),
                # which let opposite-sign bivector components read as zero rotation).
                "rotational_residual": sum(
                    math.sqrt(r.b * r.b + r.c * r.c + r.d * r.d) for _, r in dynamic
                ),
            }
        ...
        if opcode == Op.MCR_HDCU:
            # MCR-HDCU Decimal Phase Fold F₁₀: fold an integer/symbol sequence
            # into a single residue phasor (Σ mod base on the spoke wheel).
            # Read-only — delegates to mcr_hdcu, mutates no register.
            raw = operand.get("sequence")
            if not raw:
                return {"error": "operand required: sequence (list[int])"}
            try:
                seq = [int(s) for s in raw]
            except (ValueError, TypeError):
                return {"error": "sequence must be integers"}
            base = int(operand.get("base", 10))
            z = mcr_hdcu.phase_fold(seq, base)
            return {
                "label": operand.get("label", ""),
                "magnitude": abs(z),               # ~1.0 (lands on a unit spoke)
                "angle": mcr_hdcu.fold_angle(seq, base),
                "residue": mcr_hdcu.fold_residue(seq, base),
                "spoke": mcr_hdcu.fold_spoke(seq, base),
                "n": len(seq),
            }
    ...
    def snapshot_constants(self) -> dict[str, Any]:
        ...
        return {
            ...
            "entanglement_build_attosec": ENTANGLEMENT_BUILD_ATTOSEC,
            "yang_mills_gap": YANG_MILLS_GAP,
            "cosmological_lambda": COSMOLOGICAL_LAMBDA,
            "lambda_node_count": LAMBDA_NODE_COUNT,
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        center_anchor = sum(r.a for r in dynamic)  # 0_C — grade-0 scalar
        # 0_V — Cl(3,0) bivector magnitude (grade-2): per-register √(b²+c²+d²) summed
        # (rotation-invariant, cancellation-proof; was abs(Σ(b+c+d))).
        rotational_residual = sum(
            math.sqrt(r.b * r.b + r.c * r.c + r.d * r.d) for r in dynamic
        )
        return {
            "tick": self.tick,
            "cycle_position": pos,
            "toggle_torque": toggle_torque(pos),  # +7 permanent torque τ=t·sec⁴θ (never 0)
            ...
            # 370-tick dual clock: 361-tick torque ring + 9-tick observer shell.
            "phase": "observer_shell" if pos >= FORBIDDEN_TICK else "torque",
            "shell_tick": max(0, pos - FORBIDDEN_TICK),
            "torque_fraction": min(pos, FORBIDDEN_TICK) / FORBIDDEN_TICK,
            "null_ledger": self._null_ledger_signature(),
        }

    def _null_ledger_signature(self) -> dict[str, Any]:
        """Null Ledger bitwise identity on the 24-register Leech lattice grouped
        into 6 channels (6 primes x 4 states). Each channel commits a REAL bit
        (real energy-fraction >= 0.30) and an IMAGINARY bit (<= 0.70). Conserved
        iff R and I are perfect conjugates: R AND I == 0 (no mixed/torsioned
        channel) and R OR I == 0b111111 (full coverage). Canonical ideal:
        R=101010 (42), I=010101 (21), so R + I == 0b111111 and R AND I == 0."""
        R = 0
        I = 0
        ...
        for i in range(6):
            ...
            imag_e = 0.0
            for j in range(4):
                r = self.registers.get(f"R{i * 4 + j:02d}")
                if r is None:
                    continue
                real_e += r.a * r.a
                imag_e += r.b * r.b + r.c * r.c + r.d * r.d
            total = real_e + imag_e
            real_frac = (real_e / total) if total > 1e-12 else 0.5
            rb = 1 if real_frac >= 0.30 else 0
            ib = 1 if real_frac <= 0.70 else 0
            R |= rb << i
            I |= ib << i
            channels.append({"real_frac": round(real_frac, 3), "real": rb, "imag": ib})
        and_ = R & I
        or_ = R | I
        balanced = (and_ == 0) and (or_ == 0b111111)
        residual = bin(and_).count("1") + (6 - bin(or_).count("1"))
        # Binary Diagonal (Phase 43 #7) of the R channel: θ = arctan(ones/zeros) of
        # the lattice's own real-bit signature, located by its Stern-Brocot path.
        _bd = binary_diagonal.encode([(R >> i) & 1 for i in range(6)])
        return {
            "R": R,
            "I": I,
            "R_bits": format(R, "06b"),
            ...
            "balanced": balanced,
            "residual": residual,
            "canonical": {"R": 42, "I": 21, "R_bits": "101010", "I_bits": "010101"},
            "binary_diagonal": {
                "theta_deg": round(math.degrees(_bd["theta"]), 2),
                "ones": _bd["ones"],
                "zeros": _bd["zeros"],
                "stern_brocot": _bd["stern_brocot"],
                "rank": _bd["rank"],
            },
        }

    def _ternary_register_signature(
        self, triskelion: "dict[str, Any] | None" = None
    ) -> dict[str, Any]:
        """Balanced-ternary phasor view of the Triskelion gate (telemetry-only).

        Arms -> trits at the exact _classify() thresholds (triskelion.py):
          0 = arm < 0.3 (cold/weak); 1 = 0.3..0.5 (moderate); 2 = arm > 0.5 (locked).
        Phasor sum S = t_real*w^0 + t_time*w^1 + t_observer*w^2 (w = exp(2*pi*i/3)).
        balanced == True iff all three trits are equal -> S == 0 (the triadic
        sibling of the Null Ledger's 1+w+w^2 = 0 cancellation, since for equal
        trits T: S = T*(1+w+w^2) = 0). |S|, theta and dominant_arm name WHICH
        arm pulls the imbalance. shell_trit = shell_tick//3 is the 9-tick
        observer-shell sub-phase (0/1/2 = first/mid/last triad of the shell;
        positional, not a spec-named state), None in the torque phase. NOTE the
        all-cold triad 000 is balanced (S=0) the SAME as the all-locked 222 — a
        DEGENERATE balance (every arm failed the 0.3 floor); consumers should
        render it distinctly from a real lock (the HUD greys 000 as '·cold').
        Arms are real scalars; this is a discrete re-representation, not a claim
        that the gate is complex-valued. Reads no registers; mutates nothing."""
        tri = triskelion or {}
        a_r = float(tri.get("arm_real", 0.0))
        a_t = float(tri.get("arm_time", 0.0))
        a_o = float(tri.get("arm_observer", 0.0))

        def _trit(v: float) -> int:
            if v < 0.3:
                return 0
            if v <= 0.5:
                return 1
        ...
        t_r, t_t, t_o = _trit(a_r), _trit(a_t), _trit(a_o)

        half_root3 = math.sqrt(3.0) / 2.0  # 0.8660254... = Im(w)
        re_s = t_r - 0.5 * t_t - 0.5 * t_o
        im_s = half_root3 * (t_t - t_o)
        imbalance = math.hypot(re_s, im_s)
        balanced = imbalance < 1e-9

        # Direction of the imbalance — which arm pulls it. Guard the zero vector
        # so a balanced triad reports "none", not atan2(0,0)'s misleading 0 deg.
        if balanced:
            theta_deg = 0.0
            dominant_arm = "none"
        else:
            theta_deg = math.degrees(math.atan2(im_s, re_s)) % 360.0
            if theta_deg < 60.0 or theta_deg >= 300.0:
                dominant_arm = "real"
            elif theta_deg < 180.0:
                dominant_arm = "time"
            else:
                dominant_arm = "observer"

        ternary_value = t_r * 9 + t_t * 3 + t_o  # base-3 int, 0..26
        pos = self.cycle_position
        shell_trit = (pos - FORBIDDEN_TICK) // 3 if pos >= FORBIDDEN_TICK else None

        return {
            "t_real": t_r,
            "t_time": t_t,
            "t_observer": t_o,
            "trit_str": f"{t_r}{t_t}{t_o}",
            "ternary_value": ternary_value,
            "phasor_imbalance": round(imbalance, 6),  # |S|; 0.0 = balanced
            "theta_deg": round(theta_deg, 1),         # direction of imbalance
            "dominant_arm": dominant_arm,             # none|real|time|observer
            "balanced": balanced,
            "shell_trit": shell_trit,                 # 0|1|2 in shell, else None
        }

    def _resolution_kuramoto(self, retrieval: Any, scale: float = 4.0) -> dict[str, Any]:
        """Resolution fill + Kuramoto order parameter r (telemetry-only; reads no
        registers, mutates nothing).

        resolution_fill: fraction of the 144,000-vector horizon currently indexed —
          (identity_store.ntotal + knowledge_store.ntotal) / RESOLUTION_LIMIT, clamped
          to 1.0 (live FAISS ints, no I/O).
        r: Kuramoto order parameter r = |(1/N) sum exp(i*phi_k)|, phi_k = scale*theta_k,
          theta_k = ubbm_signature["theta"] per hit in [0, pi/2). scale=4 maps
          [0,pi/2)->[0,2pi) — the only scale giving genuine cancellation (r=1 iff all
          thetas equal, r->0 iff phases uniform on the circle). N = hits with a theta;
          N==0 -> r=0.0.
        mean_theta_rad: resultant phase atan2(sum sin, sum cos) in [0, 2pi).
        NOTE natural-language theta clusters near ~0.72 rad, so for real retrieval r sits
        high (~0.95 baseline) — this tracks the theta-phase CONCENTRATION of the retrieved
        set (UTF-8 ones/zeros distribution), NOT semantic or retrieval quality.
        Every label states what is MEASURED, nothing higher."""
        # RHC interpretive note (NOT emitted in the payload): in the Kuramoto
        # coupling picture r->1 reads as phase-locked retrieval and 144000 as the
        # scale where global sync is expected to transition. Those are analogies the
        # operator may apply — this function asserts only the numbers, and makes no
        # claim about hologram, consciousness, or emergence.
        from .retrieval import (  # lazy: break the chat->retrieval import cycle (cf. ledger.py)
            get_identity_store,
            get_knowledge_store,
        )

        try:
            id_n = int(get_identity_store().size)
            kn_n = int(get_knowledge_store().size)
        except Exception:  # noqa: BLE001 — FAISS not loaded yet; fail soft
            id_n = 0
            kn_n = 0
        n_states = id_n + kn_n
        fill = min(n_states / RESOLUTION_LIMIT, 1.0)

        phases: list[float] = []
        for h in list(retrieval.identity) + list(retrieval.knowledge):
            sig = h.metadata.get("ubbm_signature")
            if isinstance(sig, dict) and isinstance(sig.get("theta"), (int, float)):
                phases.append(float(sig["theta"]) * scale)
        n = len(phases)
        if n == 0:
            r = 0.0
            mean_theta = 0.0
        else:
            re_sum = sum(math.cos(p) for p in phases)
            im_sum = sum(math.sin(p) for p in phases)
            r = math.hypot(re_sum, im_sum) / n
            mean_theta = math.atan2(im_sum, re_sum) % (2.0 * math.pi)

        return {
            "resolution_fill": round(fill, 6),
            "identity_n": id_n,
            "knowledge_n": kn_n,
            "n_states": n_states,
            "resolution_limit": RESOLUTION_LIMIT,
            "r": round(r, 6),
            "mean_theta_rad": round(mean_theta, 6),
            "hit_count": n,
            "scale": scale,
        }
