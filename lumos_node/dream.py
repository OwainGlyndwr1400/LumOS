...
from .ingest import IDENTITY_INDEX, IDENTITY_MANIFEST, IDENTITY_META
    ...
    *,
    ...
    async with DREAM_LOCK:
        ...
        if not chunks_to_embed:
            ...
            state.last_run_at = datetime.now(tz=UTC).isoformat(timespec="seconds")
        ...
        try:
            ...
            merged = 0  # semantic near-dups folded into existing chunks' provenance
            ...
            for batch_start in range(0, len(chunks_to_embed), batch_size):
                ...
                for chunk, vec, anchor in zip(batch, vectors, anchors_batch, strict=True):
                    ...
                    # Phase 39 — semantic dedup: if this chunk is a near-duplicate
                    # of an EXISTING chunk (cosine >= merge_floor), merge its turn
                    # provenance into that chunk and skip the write. Opt-in; off =
                    # no-op. (Checks the live store, so same-cycle dups still pass.)
                    if settings.dedup_semantic_enabled:
                        near = identity_store.search(vec, top_k=1)
                        if near and near[0][0] >= settings.dedup_merge_floor:
                            existing = near[0][1]
                            prov = existing.setdefault("node_ids", [])
                            for nid in chunk.node_ids:
                                if nid not in prov:
                                    prov.append(nid)
                            existing["merged_turns"] = int(existing.get("merged_turns", 0)) + 1
                            merged += 1
                            continue
        ...
        # Update watermark. `added` = chunks actually embedded into the index
        # (chunks_to_embed minus any semantic near-dups merged into existing rows).
        added = len(new_metadatas)
        last_turn = pending[-1]
        ...
        state.total_consolidated += added
        state.last_run_at = datetime.now(tz=UTC).isoformat(timespec="seconds")
        state.last_run_count = added
        save_state(state, settings)
