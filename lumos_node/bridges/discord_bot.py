...
class LumosDiscordBot(discord.Client):
    ...
    def __init__(self, operator_id: int, lumos_url: str) -> None:
        ...
        # API auth — attached to every outbound call so the bridge keeps working
        # the moment the operator sets LUMOS_API_TOKEN (required for any
        # non-loopback exposure). Empty token → empty dict → unauthenticated
        # calls, byte-identical to prior behavior.
        settings = get_settings()
        self._auth_headers: dict[str, str] = {}
        if settings.api_token.strip():
            self._auth_headers["Authorization"] = f"Bearer {settings.api_token.strip()}"
        # PELE is the separate privileged-action ring; /dream/run needs it too.
        self._pele_token = settings.pele_token.strip()
        ...
        while not self.is_closed():
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout) as http:
                    async with http.get(url, headers=self._auth_headers) as resp:
                        if resp.status != 200:
    ...
    def _register_commands(self) -> None:
        ...
        async def status_cmd(interaction: discord.Interaction) -> None:
            ...
            try:
                async with aiohttp.ClientSession() as http, http.get(
                    f"{self.lumos_url}/urevm", headers=self._auth_headers
                ) as resp:
                    data = await resp.json()
        ...
        async def dream_cmd(interaction: discord.Interaction) -> None:
            if interaction.user.id != self.operator_id:
                ...
                return
            await interaction.response.defer(ephemeral=True, thinking=True)
            # /dream/run sits behind the API perimeter AND is PELE-gated, so it
            # needs the Bearer token plus the X-Lumos-PELE header. Copy the shared
            # auth dict so the PELE header never leaks onto other calls.
            dream_headers = dict(self._auth_headers)
            if self._pele_token:
                dream_headers["X-Lumos-PELE"] = self._pele_token
            try:
                async with aiohttp.ClientSession() as http, http.post(
